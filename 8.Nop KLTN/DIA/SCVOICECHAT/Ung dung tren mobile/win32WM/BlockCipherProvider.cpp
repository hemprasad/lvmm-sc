...
bool CBlockCipherProvider::CBCEncrypt(char * data,int datLen,char *Cipher)
{
	...
	char *buffN = (char*) &Nonce;
	char *buffT = (char*) &iTemp;iCount=0;
	...
	//2 ma hoa
		hashkey(key,mykey,iCount++);
		nROund=rijndaelSetupEncrypt(ck,key,128);
		rijndaelEncrypt(ck,nROund,datArray,cipArray);
		//rijndaelDecrypt(ck,nROund,cipArray,datArray);
	//3 gom file ket qua
		for (j=0;j<16;j++)
...
bool CBlockCipherProvider::hashkey(unsigned char *hashedkey,char *key,int iCounter )
{
	//tra ve mot ke tao ra tu khoa va Counter
	int len= strlen(key);
	for (int j=0;j<20;j++)
	{
		for (int i=0;i<128;i++)
		{
			if (len >i)
				hashedkey[i] = (unsigned char)( (key[i] ^ (iCounter  *3)^10) % 256);
			else
				hashedkey[i] =  (unsigned char)( ((key[i]%len) ^ iCounter  *3)^12 % 256);
		}
	}
	return	true;
	//bung kich thuoc khoa 
}