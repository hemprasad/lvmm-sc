...
CPaintDC pdc(this);
	...
	for(i=0;i<64;i++)
	p[i].CreatePen(PS_SOLID,1,RGB(60,30,120+i*2));
...
pdc->SetTextColor(RGB(0,0,255));
	...
	case 1098:case 6051:case 1099:case 1086:case 1075:case 1071:case 1100: case IDC_CHECK:  
    case IDC_RADIO1:case IDC_RADIO2:case IDC_RADIO_BLOCK: case IDC_RADIO_STREAM:
	pdc->SetBkMode(TRANSPARENT);
	HBRUSH hbr=CreateSolidBrush(RGB(50,160,254));
	return hbr;
...
//////////////////////////////////////////////////////////
				...
				{
					if (isStream)
						scpTemp2->CMDecrypt(&buff[20],length,newMesg);
					else
					{
						
						bcpTemp2->CBCDecrypt(&buff[20],length,newMesg);
					}
					for (i=0;i<length;i++)
...

{
	...
	int i,j;		
		...
		if(length<1 || length>PLAYBUFFER)
		{
		log.WriteString("\n Send : Length is more than the 200000 or less than 1");
		return;
		}	
		int size=length;
		if (isKey)
		{
			...
			if (isStream)
			{
				scpTemp1->CMEncrypt(mymesg,length,mesg);
				length %4==0?size=length:size=(length/4+1)*4;
			}
			else
			{
				bcpTemp1->CBCEncrypt(mymesg,length,mesg);
